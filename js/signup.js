...
function success(Obj, counter) {
    ...
    $('.signup').find('tips').eq(counter).hide();
...
function fail(Obj, counter, msg) {
    ...
    $('.signup').find('tips').eq(counter).text(msg).show();
    check[counter] = false;
}

// 用户名匹配
$('.signup').find('input').eq(0).change(function() {
...
$('.signup').find('input').eq(1).change(function() {
...
// 再次输入密码校验
$('.signup').find('input').eq(2).change(function() {
...
// $('.signup').find('input').eq(3).change(function() {
//     if (regPhoneNum.test($(this).val())) {
//         success($(this), 4);
//     } else {
//         fail($(this), 4, 'wizard number can only be 11 digits');
//     }
// });
...
//         success($(this), 3);
//     } else {
//         fail($(this), 3, 'phone number can only be 11 digits');
//     }
// });

//短信验证码
// var regMsg = /111111/;
// $('.signup').find('input').eq(5).change(function() {
//     if (check[4]) {
...
//         }
//     } else {
//         $('.signup').find('input').eq(4).parent().parent().removeClass('has-success').addClass('has-error');
//     }
// 
// });
...
$('#submit').click(function(e) {
    if (!check.every(function(value) {
        ...
        for (key in check) {
            if (!check[key]) {
                $('.signup').find('input').eq(key).parent().parent().removeClass('has-success').addClass('has-error')
            }
        }
    }
});
