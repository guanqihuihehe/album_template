...
/* 用于背景颜色设置------------------------------------------------------------------ */
	...
	<div id="fh5co-offcanvass">
		...
		<ul>
			<li class="active"><a href="managerindex.php">封面</a></li>
        ...
        <div id="fh5co-board" data-columns>
			<?php
				...
				if(isset($_GET["ptime1"])&&strlen($_GET["ptime1"])>0)
				{
					...
					{
						$time2=$_GET["ptime2"];
						$sql="SELECT * FROM `picture` WHERE ptime>='$time1'and ptime<='$time2' order by ptime desc";
					}
					else
					{
						$sql="SELECT * FROM `picture` WHERE ptime>='$time1' order by ptime desc";
					}
				}
				else
				{
					...
					{
						$time2=$_GET["ptime2"];
						$sql="SELECT * FROM `picture` WHERE  ptime<='$time2' order by ptime desc";
					}
					else{
	...
	picker5.on('dp.change', function (e) {
        ...
        $('#datetimepicker1').datetimepicker({
            ...
            locale: moment.locale('zh-cn'),
			<?php
			$sql='select * from picture order by ptime ';
			$connection=new mysqli("localhost","guan","gqh123","template_album");
			mysqli_query($connection, "set names utf8");
			$res=mysqli_query($connection,$sql);
			$row=mysqli_fetch_assoc($res);
		...
		function showSite()
		{
			...
			window.location="managerindex.php?ptime1="+str1+"&ptime2="+str2;
		}
		function showSite2()
		{
	...
	<script>//删除相关
		function del(path)
		{
			...
			{
				window.location="del.php?path="+path+"&ptime1="+str1+"&ptime2="+str2;
			}
		}	
	</script>
